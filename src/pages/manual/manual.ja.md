...
- **セッションの途中から始まったキャプチャには OPEN がありません。** そして UPDATE の
  エンコーディングのいくつかは、その OPEN で合意されています — AS_PATH が 2 バイト AS を
  持つのか 4 バイト AS を持つのか、各経路が ADD-PATH の Path Identifier を伴うのか。
  どちらも UPDATE 自体には現れません。BGPShark は必要なときメッセージの構造そのものから
  それを推定し、推定を事実として提示するのではなく警告バナーでそう述べます。この警告が
  出ていて、セッション確立前から取り直せるなら、取り直してください。
...
### 「ピアは複数の経路を持っているのに、1本しか届かない」 {#only-one-path-arrives-though-the-peer-has-several}

ある prefix に複数の経路を持つルートリフレクタは、ADD-PATH (RFC 7911) がネゴシエート
されていない限り、クライアントにはそのうちの1本しか送りません。そしてこのネゴシエーションは、
半分だけ正しく設定してしまいやすいものです。

**Neighbors → ルータ → セッション**、Capability Diff の下にある **ADD-PATH Result**
を読んでください。方向ごと・アドレスファミリごとに、Path Identifier が実際に送られて
いるかどうかが書かれています。

このセクションがあるのは、その上の capability の表ではこの問いに答えられないからです。
この capability は方向性を持ち、たすき掛けでネゴシエートされます — 識別子がある方向に
流れるのは、送信側が *send* を、受信側が *receive* を広告したときだけです。両端とも
「送る」設定になっているルータ同士は、同じファミリを広告し、チェックが2つ並び、晴れやかに
*"No capability mismatches detected"* と表示されます — その裏で、どちらの向きにも
Path Identifier は1つも流れていません。Result のセクションは、どちら半分が欠けているかを
名指しします。

ネゴシエートされている場合、識別子は経路の側に出ます。パケット詳細の prefix チップは
`10.1.0.0/24 path 2` と読め、経路履歴には **Path ID** 列が増えます。この列が、
*「prefix が withdraw された」* と *「2本ある経路の1本が withdraw され、もう1本は
生きている」* を分けます — これが無ければ、両者は見分けがつきません。
