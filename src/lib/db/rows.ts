/**
 * A capture, flattened into the rows of DuckDB's ten tables.
 *
 * Pure: packets in, rows out, nothing touched. It runs inside the load worker
 * (`load-worker.ts`), off the page's thread, which is the reason it is a
 * module of its own — on a capture near the size limit this is millions of
 * objects and several seconds of work that used to freeze the page while the
 * reader was trying to use it.
 *
 * Each row is an object keyed by column name. The names are the contract with
 * `schema.ts`: the insert names its columns rather than relying on position,
 * so a row with a misspelt key fails loudly instead of landing in the wrong
 * column.
 */
...
// ID counters for auto-increment. Reset at the start of every flatten, so
// ids are 1-based per capture — the tables are recreated for each load.
...
/** One table's name and its rows, in the order the tables are loaded. */
export type FlattenedTable = [table: string, rows: object[]]

/** Flatten every packet into the rows of the ten tables. */
export function flattenCapture(packets: BgpPacket[]): FlattenedTable[] {
  messageIdCounter = 0
